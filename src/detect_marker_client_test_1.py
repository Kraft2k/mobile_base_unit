...
class DetectMarkerClient(Node):
    ...
    def emergency_shutdown(self):
            self.get_logger().warn("Behavior finder markers and mergency shutdown! Spamming a Twist of 0s!")
            twist = Twist()
            twist.linear.x = 0.0
            twist.linear.y = 0.0
            twist.linear.z = 0.0
    ...
    def goal_result_callback(self, future):
        ...
        print('Получен сигнал от сервера')
        ...
        print(f'Завершение через -> {self.markers_found[_uni_marker_id].len_road_to_marker}')
        def _():
            self.detect_marker = False
            print('Завершение обратного пути')

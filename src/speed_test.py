...
class SpeedTest(Node):
    def __init__(self):
        super().__init__('test_speed')
        self.get_logger().info("Starting test speed behavior!")
        self.command = String
        self.rpm_wheel = 0.0
        ...
        self.lin_max_speed = 0.35
        self.rot_max_speed = 0.11
        self.t0 = time.time()
...
def main(args=None):
    ...
    try:
        ...
        rclpy.logging._root_logger.error('Failed to init Test speed')
        rclpy.shutdown()
        sys.exit(1)
